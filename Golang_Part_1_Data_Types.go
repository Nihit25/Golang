...
// Every language has its own advantages, same applies with Golang
package main
...
func main() {
	...
	var number uint16 = 20 //explicit declaration
	var number2 = 20       //implicit declaration (khud se smajh lega type)
  ...
  // go is a compiled language so it needs to be compiled first and then executed
	...
	// by default bool is always assigned false (0)
	// %T tells you the type of data
	// %v tells us the value in default format
	// %t - tells you boolean
	// %% - literal %
...
// So these are the basic data types in Golang 
	...
	fmt.Printf("Ypu will be %d", 2024-input) //here initially input is a string hence its gonna give an error, thats why we used strconv.ParesInt
